...
class Model(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
                  head_size=8, num_layers=3, num_labels=1, pretrained_model_path=PRETRAIN_MODEL_PATH,
    ...
    def forward(self, input_ids, attention_mask, feature_vector, GO_fea, x, attn_bias, seq_mask, graph_mask, valid=False):
        ...
        fusion_output = self.sigmoid(fusion_output)
        fusion_output = fusion_output.squeeze(dim=-1)
        return fusion_output
    


# --- Ablation --
class Model_wo_seq(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
                  ...
                  num_neurons_final=1, layer_connections=None,
                  p_drop_final=0.3,p_drop_terms=0.3):
        super(Model_wo_seq, self).__init__()
        self.model = AutoModel.from_pretrained(
            pretrained_model_path, num_labels=num_labels, problem_type="single_label_classification"
        )
        for param in self.model.parameters():
        ...
        self.fusion_layer = nn.Linear(68, 128)
        self.fc = nn.Linear(128, num_labels)
        self.relu = nn.ReLU()  
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input_ids, attention_mask, feature_vector, GO_fea, x, attn_bias, seq_mask, graph_mask, valid=False):
        ...
        # ————concat————
        vec_concat = torch.cat([graph_output, terms_output], dim=-1)
        fusion_input = vec_concat
        fusion_input = fusion_input.float()
        fusion_output = self.fusion_layer(fusion_input)
        fusion_output = self.relu(fusion_output)
...
class Model_wo_stru(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
                  ...
                  num_neurons_final=1, layer_connections=None,
                  p_drop_final=0.3,p_drop_terms=0.3):
        super(Model_wo_stru, self).__init__()
        self.model = AutoModel.from_pretrained(
            pretrained_model_path, num_labels=num_labels, problem_type="single_label_classification"
        )
        for param in self.model.parameters():
        ...
        self.fusion_layer = nn.Linear(128, 128)
        self.fc = nn.Linear(128, num_labels)
        self.relu = nn.ReLU()  
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input_ids, attention_mask, feature_vector, GO_fea, x, attn_bias, seq_mask, graph_mask, valid=False):
        ...
        # ————concat————
        vec_concat = torch.cat([seq_output, feature_vector, terms_output], dim=-1)
        fusion_input = vec_concat
        fusion_input = fusion_input.float()
        fusion_output = self.fusion_layer(fusion_input)
        fusion_output = self.relu(fusion_output)
...
class Model_wo_func(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
                  ...
                  num_neurons_final=1, layer_connections=None,
                  p_drop_final=0.3,p_drop_terms=0.3):
        super(Model_wo_func, self).__init__()
        self.model = AutoModel.from_pretrained(
            pretrained_model_path, num_labels=num_labels, problem_type="single_label_classification"
        )
        for param in self.model.parameters():
    ...
    def forward(self, input_ids, attention_mask, feature_vector, GO_fea, x, attn_bias, seq_mask, graph_mask, valid=False):
        ...
        # ————concat————
        vec_concat = torch.cat([seq_output, graph_output, feature_vector], dim=-1)
        fusion_input = vec_concat
        fusion_input = fusion_input.float()
        fusion_output = self.fusion_layer(fusion_input)
        fusion_output = self.relu(fusion_output)
...
class Model_stru(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
                  ...
                  num_neurons_final=1, layer_connections=None,
                  p_drop_final=0.3,p_drop_terms=0.3):
        super(Model_stru, self).__init__()
        self.model = AutoModel.from_pretrained(
            pretrained_model_path, num_labels=num_labels, problem_type="single_label_classification"
        )
        for param in self.model.parameters():
        ...
        self.fusion_layer = nn.Linear(36, 16)
        self.fc = nn.Linear(16, num_labels)
        self.relu = nn.ReLU()  
        self.sigmoid = nn.Sigmoid()
...
class Model_seq(nn.Module):
    ...
    def __init__(self, hidden_size=36, ffn_size=256, dropout_rate=0.1, attention_dropout_rate=0.1,
        ...
        self.fusion_layer = nn.Linear(96, 16)
        self.fc = nn.Linear(16, num_labels)
        self.relu = nn.ReLU()  
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input_ids, attention_mask, feature_vector, GO_fea, x, attn_bias, seq_mask, graph_mask, valid=False):
        ...
        # ————concat————
        vec_concat = torch.cat([seq_output, feature_vector], dim=-1)
        fusion_input = vec_concat
        fusion_input = fusion_input.float()
        fusion_output = self.fusion_layer(fusion_input)
        fusion_output = self.relu(fusion_output)
