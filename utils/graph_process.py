...
def get_edge_index(pdbdir,EPSILON = 8.):
    '''
    The pdbdir parameter is a folder path where the pdb files of samples predicted using EsmFold are stored. 
    The esmfold models should be stored in models/Protein_LLM/esmfold and can be downloaded at https://zenodo.org/records/15117305
    '''
    pdb = open(pdbdir,"r")
    atom_coordinate = []
    i = 0
...
def load_blosum():
    with open('./utils/fea_data/BLOSUM62_dim23.txt', 'r') as f:
        result = {}
        next(f)
        lines = f.readlines()
        for line in lines:
...
def load_aaphy7():
    with open('./utils/fea_data/aa_phy7', 'r') as f:
        result = dict()
        lines = f.readlines()
        for line in lines:
            line = line.strip().split()
...
if __name__ == '__main__':
    ...
    fasta_file = './data/noise_free/noise_free_train_cls.fasta'
    output_file = './data/noise_free/noise_free_graph/train_node.pkl'
    ...
    output_file = './data/noise_free/noise_free_graph/train_LPE_5_1.pkl'
    w1 = 5  
    w2 = 1  
